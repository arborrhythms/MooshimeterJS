...
var jspacklib = require('./jspack.js');
var jspack = jspacklib.jspack;
...
MeterSerializer.prototype.write = function(payload) {
    ...
    b.putBytes(payload);
    this.writer.write(new Buffer(b.getBytes()), false);
}
...
ConfigTree.prototype.enumerate = function(n,indent) {
    ...
    if(typeof(indent) == "undefined")
        indent = 0;
    print("\n");   
    print( "  ".repeat(indent) + n.toString() );
...
ConfigTree.prototype.pack = function() {
        ...
        print("PLAIN BYTES: " + plain.length);
        var compressed = zlib.pack(plain);
        print("COMPRESSED BYTES: " + compressed.length);
        return compressed;
}
function ab2str(buf) {
...
BytePack.prototype.putFloat = function(v, b) {
    b = (typeof(b)=="undefined") ? 1 : b;
    v = jspack.Unpack("BBBB", jspack.pack("f",v));
    this.putBytes(v);
}
...
BytePack.prototype.get = function(b, signed, t) {
    ...
    if(t == "int") {
        ...
        if(4 > this.getBytesRemaining())
            throw "UnderflowException";
        r = jspack.Unpack("f", jspack.Pack("BBBB", this.bytes.slice(this.i,this.i+4) ));
        this.i += 4;
        return r[0];
    }
    else {
...
module.exports = {
    ...
    'CRC':        CRC,
    'print':      print,
    'MeterSerializer': MeterSerializer
};
